...
function collectErrorChain(error: unknown): unknown[] {
	...
	while (queue.length > 0) {
		...
		if (isObjectLike) {
			...
			if ("cause" in details && details.cause != null) {
			...
			if ("lastError" in details && details.lastError != null) {
				queue.push(details.lastError);
			}
		}
	}
	return results;
...
function buildKeywordErrorMessage(error: Error): string {
	const chain = collectErrorChain(error);

	const hasConnectTimeout = chain.some((item) => {
		if (item instanceof Error) {
			const message = item.message.toLowerCase();
			if (message.includes("connect timeout") || message.includes("timeout")) {
				return true;
			}
		}
	...
	const nestedMessage = chain
		...
		.find(
			(message) =>
				message && message !== error.message && message !== "分析文本失败",
		);

	if (nestedMessage) {
		return `关键词分析失败：${nestedMessage}`;
	}

	return "关键词分析失败：请稍后重试。";
}
...
export async function getKeywords(
	...
	const billingEmail = session?.user?.email ?? billingEmailOverride ?? null;

	// 2. 如果未登录且没有提供用户 API key，检查是否超出访问限制
	if (!isAuthenticated && !userApiKey && !billingEmail) {
	...
	if (error) {
		...
		return {
			error: {
				code: "AI_ERROR",
				message: buildKeywordErrorMessage(error),
			},
		};
	}
	...
	if (billingEmail) {
		...
		if (totalTokens > 0) {
			try {
				const billingResult = await chargeUsageForTokens(
					billingEmail,
					totalTokens,
				);
				billingInfo = {
					costCharged: billingResult.cost,
					remainingBalance: billingResult.balance,
				};
			} catch (billingError) {
				const message =
